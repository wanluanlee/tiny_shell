...
void eval(char *cmdline)
{
    ...
    numCommand = parseargs(argv,commands,stdin_redir,stdout_redir);
    sigset_t mask_all, prev_all;
    sigfillset(&mask_all);
    int pipfd[numCommand-1][2];
    //printf("here2\n");
    if(argv[0] == NULL)
        return;
    ...
    if(!builtin_cmd(argv))
    {
        ...
        for(int i = 0; i < numCommand; ++i) {
            //            printf("stdin_redir[i]: %d\n", i);
            //            printf("stdout_redir[i]: %d\n", i);
            //            printf("%s\n", argv[i]);
            ...
            if (numCommand > 1) {
                ...
                if (pid == 0) {
                    ...
                    if (stdin_redir[i] >= 0) {
                        close(1);
                        FILE *fptr;
                        fptr = fopen(argv[stdin_redir[i]], "r");
                        //fprintf(stderr, "in %d\n",fileno(fptr));
                        dup2(fileno(fptr), 0);
                    ...
                    if (stdout_redir[i] >= 0) {
                        close(0);
                        FILE *fptr;
                        fptr = fopen(argv[stdout_redir[i]], "w");
                    ...
                    setpgid(0, 0);
                    if (execve(argv[commands[i]], &argv[commands[i]], NULL) < 0) {
                        //printf("command: %d\n", commands[i]);
        ...
        if (!bg)
        {
            addjob(jobs, pid, temp_pid, FG, cmdline);
            sigprocmask(SIG_SETMASK, &prev_all, NULL);
            waitfg(pid);
        } else{
            addjob(jobs, pid, temp_pid, BG, cmdline);

            printf("[%d] (%d) ", getjobpid(jobs,pid)->jid, getjobpid(jobs,pid)->pid);
            printf("%s", getjobpid(jobs,pid)->cmdline);
            sigprocmask(SIG_SETMASK, &prev_all, NULL);
        }
    }
...
void do_bgfg(char **argv)
{
    ...
    if(!strcmp(argv[0],"bg"))
    {
        ...
        {
            jid = argv[1]+1;
            xmax = strtoimax(jid, &tmp, 10);
            newJid = (int)xmax;
            if(getjobjid(jobs,newJid) != NULL)
            {
                pid = getjobjid(jobs,newJid)->pid;
                kill(pid, SIGCONT);
    ...
    if(!strcmp(argv[0], "fg"))
    {
        if(firstChar[0] == '%')
        {
            ...
            if(getjobjid(jobs,newJid) != NULL)
            {
                ...
                //printf("%s %d \n", "my pid",pid);
                kill(-pid, SIGCONT);
                getjobjid(jobs,newJid)->state = FG;
                waitfg(pid);
                //printf("[%d] (%d) ", getjobpid(jobs, pid)->jid, getjobpid(jobs, pid)->pid);
                //printf("%s", getjobpid(jobs, pid)->cmdline); 
            }
...
void sigchld_handler(int sig)
{
    ...
    while((pid = waitpid(-1,&status,WUNTRACED|WNOHANG)) > 0) {
        ...
        if (WIFSIGNALED(status)) {
            sigprocmask(SIG_BLOCK, &mask_all, &prev_all);
            //printf("%s %d \n", "Before delete", pid);
            fprintf(stdout, "%s [%d] (%d) ", "Job ", getjobpid(jobs, pid)->jid, getjobpid(jobs, pid)->pid);
            fprintf(stdout, " terminated by signal 2\n");
            deletejob(jobs, pid);
            //printf("%s %d \n", "After delete", pid);
        ...
        if (WIFSTOPPED(status)) {
            ...
            fprintf(stdout, "%s [%d] (%d) ", "Job ", getjobpid(jobs, pid)->jid, getjobpid(jobs, pid)->pid);
            fprintf(stdout, " stopped by signal 20 \n");
        }
